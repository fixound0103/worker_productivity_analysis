...
# 2. 파일 업로드 및 기준 초 입력 섹션 (accept_multiple_files=True 설정으로 다중 업로드 지원)
    ...
    if st.button("입고 입력 생산성 분석 시작"):
        try:
            ...
            with st.spinner(f'데이터 정제 및 {target_seconds}초 기준 생산성 지표를 계산 중입니다...'):
                ...
                # 파일 이름 순서대로 정렬 (예: 01일, 02일, 03일 순서 정렬 보장)
                sorted_files = sorted(uploaded_files, key=lambda x: x.name)

                # 파일명 요약 리스트 (다운로드 파일명 생성용)
                file_names_summary = []
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    ...
                    for uploaded_file in sorted_files:
                        ...
                        # [시트 2 데이터 정의]
                        sheet2_df = unique_df.sort_values(by=['최초입고처리자', '최초입고처리시간'],
                        ...
                        sheet3_df = pd.concat(columns_to_combine,
                                              axis=1) if columns_to_combine else pd.DataFrame()  # 시트 3 데이터

                        # =====================================================================
                        # [단계 3] 파일명 접두사를 시트이름에 포함하여 저장
                        # =====================================================================
                        # 엑셀 시트 글자수 제한(31자)을 넘지 않도록 상세작업시간 시트명 최적화
                        sheet1_df.to_excel(writer, sheet_name=f'{base_file_name}_생산성분석', index=False)
                        sheet2_df.to_excel(writer, sheet_name=f'{base_file_name}_작업자별 정렬', index=False)
                        sheet3_df.to_excel(writer, sheet_name=f'{base_file_name}_상세작업시간', index=False)
